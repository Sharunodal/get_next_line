...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/06 11:16:05 by arissane          #+#    #+#             */
/*   Updated: 2024/06/06 11:19:18 by arissane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1 || fd > OPEN_MAX)
		return (NULL);
	saved_text[fd] = read_line(fd, saved_text[fd]);
	if (saved_text[fd] == NULL)
		return (NULL);
	line = get_line(saved_text[fd]);
	if (line == NULL)
		return (free_address(&saved_text[fd]));
	saved_text[fd] = new_line(saved_text[fd]);
	return (line);
}
